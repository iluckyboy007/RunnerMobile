...
const MobileControls: React.FC = () => {
    const { status, hasImmortality, isImmortalityActive } = useStore();
    if (status !== GameStatus.PLAYING) return null;

    const btnBase = "flex items-center justify-center rounded-full bg-white/5 backdrop-blur-lg border transition-all active:scale-90 active:bg-white/10 pointer-events-auto touch-none shadow-lg";
    
    return (
        <div className="absolute inset-0 pointer-events-none flex flex-col justify-end p-6 z-[60] select-none">
            <div className="flex justify-between items-end w-full max-w-5xl mx-auto">
                {/* Left: Movement */}
                <div className="flex space-x-4">
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('player-move-left')); }}
                        className={`${btnBase} w-16 h-16 border-cyan-500/30 shadow-cyan-500/10`}
                    >
                        <ArrowLeft className="w-8 h-8 text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
                    </button>
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('player-move-right')); }}
                        className={`${btnBase} w-16 h-16 border-cyan-500/30 shadow-cyan-500/10`}
                    >
                        <ArrowRight className="w-8 h-8 text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
                    </button>
                </div>
                ...
                <div className="flex flex-col items-end space-y-4">
                    {hasImmortality && (
                        <button 
                            onPointerDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('player-use-ability')); }}
                            className={`${btnBase} w-14 h-14 border-yellow-500/50 ${isImmortalityActive ? 'bg-yellow-500/20' : 'bg-black/20'}`}
                        >
                            <Shield className={`w-6 h-6 ${isImmortalityActive ? 'text-white' : 'text-yellow-400'} drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]`} />
                        </button>
                    )}
                    <button 
                        onPointerDown={(e) => { e.preventDefault(); window.dispatchEvent(new Event('player-jump')); }}
                        className={`${btnBase} w-20 h-20 border-purple-500/50 shadow-purple-500/20`}
                    >
                        <ArrowUpCircle className="w-10 h-10 text-purple-400 drop-shadow-[0_0_12px_rgba(168,85,247,0.8)]" />
                    </button>
                </div>
            </div>
        </div>
