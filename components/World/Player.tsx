...
export const Player: React.FC = () => {
  ...
  // Listen for Custom Events (from Virtual HUD)
  useEffect(() => {
    window.addEventListener('player-move-left', moveLeft);
    window.addEventListener('player-move-right', moveRight);
    window.addEventListener('player-jump', triggerJump);
    window.addEventListener('player-use-ability', activateImmortality);

    return () => {
        window.removeEventListener('player-move-left', moveLeft);
        window.removeEventListener('player-move-right', moveRight);
        window.removeEventListener('player-jump', triggerJump);
        window.removeEventListener('player-use-ability', activateImmortality);
    };
  }, [laneCount, hasDoubleJump, activateImmortality]);
  ...
  // Swipes (kept for dual-control flexibility)
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        // Skip swipe detection if the event originated from a button (pointer-events-auto on buttons)
        // This is handled by HUD dispatching custom events and being "pointer-events-auto"
